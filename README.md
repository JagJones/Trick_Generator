...
It's also meant to both look and act in a minimalistic manner.
...
. Flexbox was used throughout the entire project, except in the hamburger menu. (More info in the @media sections below.)
    ...
    - At the largest viewport size the main conent should seperate into 3 columns. The contents of the first column should wrap.
. CSS was used to create shapes, the little lavender dots to seperate some paragraphs and content.
...
. There's 2 sizes of @media used in this project: 
    - One is a (max-width: 724px)
    . The navigation switches from a hamburger menu to a simple colum layout.

    - The other is (min-width: 1025px)
    .The navigation switches to one line.
    . The image in the main content gets larger. 
    . Main content breaks into 3 columns. First column wraps to fill the pg height, the other 2 are supposed to be realitively even and aligned next to each other. 
